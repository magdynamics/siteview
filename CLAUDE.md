...
- `backend/.env` (copy from `.env.example`): `FIREBASE_PROJECT_ID`, `FIREBASE_PRIVATE_KEY`, `FIREBASE_CLIENT_EMAIL`, `FIREBASE_STORAGE_BUCKET`, `GOOGLE_MAPS_API_KEY`, `PORT`, `FRONTEND_URL` (CORS origin), `ANTHROPIC_API_KEY` (optional — enables `services/nlp.js`), `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_FROM_NUMBER` (optional — enables `services/sms.js`; without them `POST /api/notifications/sms` logs and reports `skipped: true` per recipient instead of failing). Service account key lives at `backend/serviceAccountKey.json` (never commit).
